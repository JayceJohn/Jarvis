...
const TypewriterLine = React.memo(
  ({ line, color }: { line: string; color: string }) => {
...
const App: React.FC = () => {
  ...
  const [activeMode, setActiveMode] = useState<"chat" | "image" | "vision">(
    "chat",
  );
  ...
  const [visionMethod, setVisionMethod] = useState<
    "screen" | "camera" | "mirror" | null
  >(null);
  ...
  useEffect(() => {
    ...
    const initial = Array.from({ length: 30 }).map((_, i) => ({
      time: new Date(Date.now() - (30 - i) * 20000).toLocaleTimeString([], {
        minute: "2-digit",
        second: "2-digit",
      }),
    ...
    const interval = setInterval(() => {
      setMetricsHistory((prev) => {
        ...
        const nextGpu = Math.max(
          5,
          Math.min(95, lastGpu + (Math.random() * 16 - 8)),
        );
        const nextMs = Math.max(
          10,
          Math.min(300, lastMs + (Math.random() * 40 - 20)),
        );
  ...
  useEffect(() => {
    // Process any returning redirect logins (from WebView Fallbacks)
    getRedirectResult(auth).catch((error) => {
        console.error("Redirect Error:", error);
        alert(`Redirect Authentication failed: ${error.message}\n\nIf you use an in-app browser or incognito mode, try opening this link in standard Safari or Chrome.`);
    });
    ...
    return onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (userDoc.exists()) {
          ...
          setUserPfp(
            currentUser.photoURL || "https://picsum.photos/seed/user/100/100",
          );
        }
  ...
  const startScreenShare = async () => {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia) {
        throw new Error(
          "Screen sharing is not supported in this browser environment. This often happens inside preview windows or on mobile. Please use 'Mirror Mode' or open Jarvis in a New Tab.",
        );
      }
  ...
  const handleSend = async () => {
    ...
    try {
      const currentLanguage =
        LANGUAGES.find((l) => l.code === themeSettings.language) ||
        LANGUAGES[0];
      ...
      if (attachedImage) {
        ...
        userMsg.imageUrl = await compressImage(
          attachedImage,
      ...
      if (currentAttached) {
        ...
        for await (const chunk of result) {
          ...
          if (!user)
            setMessages((prev) =>
              prev.map((m) =>
                m.id === jarvisId ? { ...m, text: jarvisFinalText } : m,
              ),
            );
        }
        ...
        for (const part of response.candidates[0].content.parts) {
          if (part.inlineData) {
            ...
            responseMsg.imageUrl = await compressImage(
              croppedImg,
              STORAGE_IMAGE_MAX_WIDTH,
              STORAGE_IMAGE_QUALITY,
            );
        ...
        if (!frameData) {
          responseMsg.text =
            "I can't see anything! Please ensure Vision mode is properly active.";
          jarvisFinalText = responseMsg.text;
          if (!user) {
            const jarvisId = Date.now() + 1;
            setMessages((prev) => [...prev, { ...responseMsg, id: jarvisId }]);
      ...
      if (
        ...
        errorText =
          "Access Denied: Please ensure you have selected a valid API key with the required AI feature permissions.";
        setShowApiKeyPrompt(true);
      } else if (e.message?.includes("404")) {
        errorText =
          "Model not found. This feature might not be available in your region.";
      }
      setMessages((prev) => [
        ...prev,
        {
  ...
  const requestPiP = async () => {
    try {
      if (videoRef.current && document.pictureInPictureEnabled) {
        ...
        alert(
          "Picture-in-Picture is not supported in this browser. Try Chrome or Safari.",
        );
      }
    } catch (err) {
  ...
  const handleLogin = async () => {
    try {
      if (navigator.userAgent.includes("wv") || navigator.userAgent.includes("Instagram") || navigator.userAgent.includes("FBAV") || navigator.userAgent.includes("Snapchat")) {
          // If explicitly in a WebView (Internet App / Social Media) just try redirecting directly 
          // because popups will 100% fail and might not even return an error.
      ...
      // If the browser blocked the popup, fallback to Redirect method
      if (
        ...
        alert(
          `Domain Not Authorized!\n\nYou must explicitly add this website's URL to your Firebase Console -> Authentication -> Settings -> Authorized Domains.`,
        );
      } else {
      ...
      style={
        {
          ...
          "--theme-secondary": themeSettings.useGradient
            ? themeSettings.secondary || themeSettings.primary
        ...
        {appReady && (
          ...
          >
            ...
            <div className="fixed inset-0 pointer-events-none opacity-20 overflow-hidden z-0">
              ...
              <div
                className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-[var(--theme-secondary)] blur-[120px] animate-pulse"
                ...
                >
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-xs">
                        Choose how Jarvis observes your world
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-4">
                    ...
                    <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 group hover:border-[var(--theme-primary)]/30 transition-all">
                      ...
                      <button
                        onClick={() =>
                          setThemeSettings((prev) => ({
                            ...prev,
                            alwaysRememberVision: !prev.alwaysRememberVision,
                          }))
                        }
                      ...
                      >
                        <div
                          className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${themeSettings.alwaysRememberVision ? "translate-x-5" : ""}`}
                        ></div>
                      </button>
                    </div>
                    ...
                    >
                      ...
                      <div>
                        <div className="font-bold text-white uppercase tracking-widest text-sm">
                          Full Screen Mode
                        </div>
                        <div className="text-xs text-slate-500">
                          Best for Desktop. Jarvis sees other apps/tabs.
                        </div>
                        <div className="mt-1 text-[10px] text-blue-400 font-bold uppercase tracking-tighter">
                          * Requires new tab if on mobile/preview
                        </div>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500">
                          Best for Mobile. Jarvis sees through your camera.
                        </div>
                      </div>
                    </button>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-amber-500/80 font-bold uppercase mt-0.5">
                          Note: Higher latency than Screen Share
                        </div>
                      </div>
                    </button>
                  </div>
                  ...
                  <div className="mt-8 flex flex-col gap-3">
                    ...
                    <p className="text-[9px] text-slate-600 text-center uppercase tracking-widest leading-relaxed">
                      * Ensure your Gemini API Key is selected in the{" "}
                ...
                <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl relative z-10">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <div className="flex justify-between mb-2">
                          ...
                          <span className="text-xs font-mono text-cyan-400">
                            {Math.round(
                              themeSettings.imageStorageQuality * 100,
                            )}
                            %
                          </span>
                        </div>
                        <input
                          ...
                          onChange={(e) =>
                            setThemeSettings((prev) => ({
                              ...prev,
                              imageStorageQuality: parseFloat(e.target.value),
                            }))
                          }
                      ...
                      <div>
                        <div className="flex justify-between mb-2">
                          ...
                          <span className="text-xs font-mono text-cyan-400">
                            {themeSettings.imageStorageWidth}px
                          </span>
                        </div>
                        <input
                          type="range"
                          ...
                          onChange={(e) =>
                            setThemeSettings((prev) => ({
                              ...prev,
                              imageStorageWidth: parseInt(e.target.value),
                            }))
                          }
                          className="w-full accent-[var(--theme-primary)]"
                        />
                        <p className="text-[10px] text-slate-500 mt-2 leading-relaxed">
                          Warning: Adjusting this above 2048px may exceed
                          Firebase cloud saving limits, preventing chat logic
                          from persisting.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
                      ...
                      >
                        <div
                          className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${themeSettings.alwaysRememberVision ? "translate-x-6" : ""}`}
                        ></div>
                      </button>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <div
                          className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${themeSettings.useGradient ? "translate-x-6" : ""}`}
                        ></div>
                      </button>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between mb-2">
                        ...
                        <span className="text-sm font-mono text-white">
                          {Math.round(themeSettings.opacity * 100)}%
                        </span>
                      </div>
                      <input
                        type="range"
              ...
              <div className="fixed inset-0 z-[300] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 text-center">
                <div className="bg-slate-900 border border-red-500/30 rounded-2xl p-8 w-full max-w-md shadow-2xl">
                  ...
                  <h2 className="text-2xl font-bold mb-4 text-white uppercase tracking-wider">
                    Vision Feature Required
                  </h2>
                  <p className="text-slate-400 mb-8 leading-relaxed">
                    To use real-time screen analysis, Jarvis needs access to
                    your screen. Please enable screen sharing mode.
                  </p>
                ...
                <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-sm relative z-10">
                  ...
                  <div className="space-y-4">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 outline-none focus:border-[var(--theme-primary)] transition-colors"
                    ...
                    <button
                      onClick={saveProfile}
                      className="w-full bg-red-600 hover:bg-red-500 text-white font-bold py-2.5 rounded-xl transition-colors uppercase tracking-widest mt-2 shadow-lg shadow-red-500/20"
                    >
                      Save Integrity
                    </button>
                    <button
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-1">
                      ...
                      {userChats.length > 0 ? (
                        userChats.map((chat) => (
                          ...
                          >
                            <div
                              className={`flex items-center gap-3 transition-transform duration-300 ${holdingChatId === chat.id ? "-translate-x-24" : ""}`}
                            >
                              <MessageCircle
                                size={16}
                                className={
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                          ...
                          <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-2 leading-relaxed">
                            Start your first session to begin your encrypted
                            history.
                          </p>
                        </div>
                      )}
                ...
                <>
                  ...
                  <div className="fixed inset-0 z-[601] pointer-events-none flex items-center justify-center p-4">
                    <motion.div
                      ...
                      style={{
                        borderColor: `${themeSettings.primary}66`,
                        boxShadow: `0 0 80px ${themeSettings.primary}15`,
                      }}
                    >
                      <div className="relative z-10 p-6 lg:p-8">
                        <div
                          className="flex justify-between items-start mb-8 pb-4"
                          style={{
                            borderBottom: `1px solid ${themeSettings.primary}44`,
                          }}
                        >
                          <div className="flex items-center gap-4">
                            <Activity
                            ...
                            />
                            <div>
                              <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-[0.2em] leading-none">
                          ...
                          <button
                            onClick={() => setIsSystemStatusOpen(false)}
                            className="text-slate-500 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/5 border border-transparent hover:border-white/10"
                          >
                            <X size={24} />
                          </button>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                          <div
                            className="bg-black/40 border p-6 rounded-2xl flex items-center gap-4"
                            style={{
                              borderColor: `${themeSettings.primary}33`,
                            }}
                          >
                            <div
                              className="p-3 bg-black/50 rounded-xl border"
                              style={{
                            ...
                            <div>
                              ...
                              <p className="text-green-500 text-lg font-black uppercase tracking-widest">
                                ONLINE
                              </p>
                            </div>
                          </div>
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p
                                className={`text-lg font-black uppercase tracking-widest ${metricsHistory.length > 0 ? getMsColorClass(metricsHistory[metricsHistory.length - 1].ms) : "text-green-500"}`}
                              >
                                {metricsHistory.length > 0
                                  ? metricsHistory[metricsHistory.length - 1].ms
                                  : 0}{" "}
                                MS
                              </p>
                            </div>
                    ...
                    <>
                      <div className="flex items-center gap-1">
                        ...
                        >
                          <Activity
                            ...
                            className={
                              metricsHistory.length > 0 &&
                              metricsHistory[metricsHistory.length - 1].ms > 150
                      ...
                      >
                        <img
                          src={userPfp}
                          alt={userName}
                          className="w-8 h-8 rounded-full object-cover border border-slate-700 group-hover:border-[var(--theme-primary)] transition-colors"
                        />
                      </div>
                      <button
            ...
            <main className="flex-1 flex flex-col w-full max-w-5xl mx-auto p-4 min-h-0 relative z-10">
              <div className="flex-1 overflow-y-auto mb-4 space-y-4 pr-1">
                {messages.length === 0 && !isLoading && (
                  <div className="flex flex-col items-center justify-center h-full text-center space-y-6 animate-in fade-in zoom-in duration-500">
                    ...
                    >
                      Jarvis is ready for chat, image generation, or real-time
                      screen analysis.
                    </motion.p>
                    <div className="flex flex-wrap justify-center gap-2">
                      {[
                        "Enable screen share mode",
                  ...
                  >
                    ...
                    {msg.imageUrl && (
                      <div className="flex flex-col gap-2">
                        <img
                          src={msg.imageUrl}
                          alt="Generated"
                          className="rounded-lg max-w-full shadow-lg border border-white/5"
                        />
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() =>
                              downloadFile(msg.imageUrl, "jarvis-image.png")
                            }
                    ...
                    {msg.audioUrl && (
                      <div className="flex flex-col gap-2">
                        ...
                        <button
                          onClick={() =>
                            downloadFile(msg.audioUrl, "jarvis-audio.wav")
                          }
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-2 flex items-center gap-1 sm:gap-2 shrink-0">
                ...
                <button
                  ...
                  style={{
                    color:
                      isScreenSharing || activeMode === "vision"
                        ? themeSettings.primary
                        : "#64748b",
                  }}
                ...
                <input
                  ...
                  className="flex-1 bg-transparent outline-none p-2 text-sm sm:text-base min-w-0"
                  placeholder={
                    activeMode === "chat"
                      ? "Talk to Jarvis..."
                      : `Ask about ${activeMode === "vision" ? "screen" : "image"}...`
                  }
                />
                <button
                  onClick={handleSend}
                ...
                <div className="fixed bottom-10 left-10 z-[1000] flex items-end gap-4 pointer-events-none">
                  ...
                  >
                    <div className="relative group">
                      <motion.div
                        animate={{
                          scale: isBubbleChatOpen ? 1.2 : [1, 1.1, 1],
                        }}
                        transition={{
                          duration: 2,
                          repeat: isBubbleChatOpen ? 0 : Infinity,
                        }}
                    ...
                    >
                      ...
                      <div className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth">
                        {messages.slice(-8).map((msg, i) => (
                          <div
                            key={i}
                            className={`flex flex-col ${msg.senderId === "jarvis" ? "items-start" : "items-end"}`}
                          >
                            <div
                              className={`max-w-[90%] p-3 rounded-2xl text-[13px] leading-relaxed shadow-sm ${
                                msg.senderId === "jarvis"
